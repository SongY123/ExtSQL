# ExtSQL PostgreSQL 18.4 Extension Image

This image is based on `postgres:18.4` and provides the PostgreSQL extension set needed by ExtSQL-Bench. It installs the extension binaries and control files, and packages the initialization SQL at `/docker-entrypoint-initdb.d/10-enable-extensions.sql`. When PostgreSQL initializes an empty data directory for the first time, these extensions are enabled automatically.
...
To customize the image tag or pass extra `docker build` options:
...
You can also set extension package versions through environment variables:

```bash
PG_SEARCH_VERSION=0.24.1 PG_JSONSCHEMA_VERSION=0.3.4 scripts/build_image.sh
```

The equivalent direct Docker command from the `image` directory is:
...
`timescaledb` and `pg_search` require `shared_preload_libraries`. The Dockerfile writes this setting into the PostgreSQL configuration template. The startup flags above also make it effective when an existing data directory overrides the generated configuration.

## Initialize Extensions

The image already includes `sql/enable_extensions.sql`, and PostgreSQL runs it automatically when initializing an empty `PGDATA` directory. Existing data directories do not rerun `/docker-entrypoint-initdb.d`. To enable the extensions manually in an existing database, run:

```bash
docker exec -i extsql-postgres psql -U postgres -d postgres < sql/enable_extensions.sql
```

`/docker-entrypoint-initdb.d` only runs during initialization of an empty `PGDATA` directory.

## Extension Mapping and SQL

| Type | Extension | Source | SQL |
| --- | --- | --- | --- |
